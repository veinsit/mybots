...
const displayAtleti = (chat, cam, atleti: any[]) =>
    chat.say("Ecco gli atleti della squadra:").then(() =>
        chat.sendGenericTemplate(
            atleti.map((currElement, index) =>
                atletaTemplateElement(currElement, cam)
            ), //elements, 
...
function atletaTemplateElement(a, cam): any {
    return {
...
const showHelpPingPong = (chat) =>
    chat.say(
        `In ogni momento, puoi scrivere "squadre", oppure "squadra" seguita dal codice della squadra Ad esempio: "squadra 7401".`
        , { typing: true }
